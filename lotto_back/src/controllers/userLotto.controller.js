...
export class UserLottoController {
  ...
  getUserLottosByUUID = async (req, res) => {
    const { H_U_I_1 } = req.cookies;
    const lottos = await this.userLottoService.getUserLottosByUUID(H_U_I_1);
    res
      .status(200)
      .json(ResponseUtil.success(lottos, "User lottos retrieved successfully"));
  };

  getAllUserLottos = async (req, res) => {
